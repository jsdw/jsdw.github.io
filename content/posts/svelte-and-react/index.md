...
I've been using React professionally for around 2 years now. A few weeks ago I was linked to an [excellent talk][svelte-talk] called "Rethinking Reactivity" by Rich Harris, the creator of Svelte. The talk spends some time contrasting Svelte and React, and compelled me to give Svelte a try myself. Since then, I've created a couple of small Svelte based interfaces, and it's time to reflect on how I feel about Svelte so far, and contrast it with React.
...
The "starter" templates I find for React don't tend to align so well with my personal preferences. They tend to ship with a bunch of dependencies I don't care for in the beginning, but also miss some things that I do care about. I usually end up installing Parcel and manually adding TypeScript and setting up scoped CSS before I can begin (note to self: I should really just go and create myself a starter template to avoid this hassle in the future).

It's a big deal for me that I can get up and running in the way I like with Svelte so easily (and just by using the recommended approach from the Svelte site no less). Given the complexity of build systems and time I generally seem to waste getting a project set up the way I like, it's lovely to find a framework who's starting point is almost perfect for me from the get go. I hope this remains the case.


# Compile-time vs Runtime Reactivity

The most significant difference and thing that compelled me to give Svelte a go in the first place is that Svelte will generate code at compile-time to know what to update and when, whereas React will use virtual DOM diffing at runtime to achieve the same.

Svelte promises much smaller bundles and faster code as a result of pushing a bunch of this work to the compile stage and so not needing to ship with anything like a virtual DOM implementation.
...
Svelte starts from the other end of the spectrum. At compile-time, Svelte will instrument and generate the code needed to mutate precisely what it needs to and nothing more each time something changes. Because the code to handle the reactivity is generated at compile time, Svelte doesn't rely on doing DOM diffing at runtime, and doesn't need to ship the supporting code to perform it. This leads to a reduction in bundle size (no need for a Virtual DOM implementation) and in speed (no runtime diffing).

So, in Svelte, instead of annotating those computations you wish to avoid re-running, you end up annotating the things that you _do_ want to re-run:
- Values prefixed with `$:` will be recalculated if any of the variables mentioned in the expression are known to have changed themselves. `$:` can prefix blocks, things like `if` statements, or variable assignments.
- Any assignment to a value is instrumented at compile-time so that Svelte knows to update dependencies (`$:` things that mention the value for instance).
...
Here, calls to `count += 1` are instrumented at compile time to update anything depending on `count`, and `totalCount` will be updated each time `count` or `parentCount` are, since we are using `$:` and Svelte can see that `count` and `parentCount` are depended on here.

The upside of the Svelte approach is performance; updates are performed with surgical precision. It's a bit like everything being automatically surrounded in `useMemo` and `useCallback` and so on for you.
...
Here, Svelte no longer updates `totalCount`, because the line updating it no longer references `count` or `parentCount`, and so Svelte doesn't know that `totalCount` depends on those. It's a bit like forgetting to include a dependency in a call to `useMemo` in React; dependencies have to be mentioned in some way for either framework to know that an expression needs re-running.
...
Svelte has much smaller bundle sizes and benchmarks (and very limited anecdotal evidence) support the idea that it's a bunch faster than React, owing to doing more at compile time.

I'm generally a big fan of the fact that the default in Svelte is going to be fast, whereas in React you need to `useMemo` and such in order to make your code more optimal (and generally think more carefully about things from a performance point of view). `useMemo` and co are pretty hideous, and it's easy to forget to manually list all of the dependencies for them. There's probably a lint for that, but I haven't taken the time to set it up yet.

Perhaps the small bundle sizes excite me even more than the speed. I could see myself using Svelte for small interactive components in an otherwise static website for instance, or for building a component library with Svelte alongside adapters to make it work with React, Vue etc. The fact that the output from Svelte is so low in overhead (I imagine that writing Svelte like components in raw JS would lead to not-much-smaller-if-at-all bundle sizes) makes this a compelling option. I just wouldn't think to use React in quite the same way as it's size makes it a bigger commitment.
...
Not needing hooks is great, it turns out. Hooks [come with rules][hook-rules] which limit how they can be used, and can be tricky to compose, but plain old functions can be composed and reused any which way you like. Certain Svelte functions do need to be called synchronously with the component being initialised however, and I wish that those followed a naming convention like hooks do, so that it was clear exactly what they were (and you could go and apply this same convention to functions that compose these). I might stick to pre-pending `use` to such functions, or eventually settle on another word that would be a good fit here.
...
This does help to highlight how well thought through the `useEffect` API is (I think, at least); in Svelte we are perfectly able to emulate the same behaviour, but we have to manually handle resubscribing as needed, and cleanup when the component is destroyed.
...
I'll have to think about whether this is a good idea or not, but needless to say, I think that Svelte has the ability to emulate the sorts of things we can do with hooks in React (and perhaps better).


# Component Layout

In Svelte, the HTML(-like) description, code, and styles live in a single file. An empty file is a valid Svelte component, so it's pretty light weight. Since this setup comes out of the box I can get up and running very quickly, which is lovely.

Using React, I have to mess around with the build system to settle on a React based workflow that does what I want (eg scoped CSS), which is a hinderance.

That said, React ultimately offers more flexibility here; you can configure plugins to give you inline CSS, and you can define multiple components in a single file. Code that is global to components is also natural and easy to add; you can have the same in Svelte but it's not as immediately obvious how.

In React, I'd often incrementally break down a component into smaller components in the same file while I was iterating on it, and later I might decide to move those out to separate files. I miss this a little in Svelte; components in Svelte _are_ lightweight, but having to make a separate file and import it each time I want to break anything down is more of a hinderance than defining a new function. I feel like I've tended towards slightly larger Svelte components as a result.
...
I planned to write this section pointing out that this couldn't be well typed either, but it turns out that I was wrong! By default, the `message` event dispatched isn't super well typed (the event value defaults to `any` which makes me a bit sad; I'd rather it was `unknown`). However, I can change the dispatch line to:
...
So far, I've just defaulted to passing functions down to be called instead of using this message interface, since it's well typed by default and I don't generally want to expose other event like properties. I'll have to explore this area a little further to get a good feeling of where the typing is better and worse and what approach to take coming from React.
...
I've so far avoided using this myself. I've run into plenty of issues using two way bindings from my time with Angular, and prefer the explicit approach React takes. That said, I can't deny that binding can reduce boilerplate, and so with time I expect I'll find a happy medium when using Svelte.

One thing that's lovely about Svelte in this area is that it provides built-in bindings for things like obtaining the width and height of an element, which is known to be annoying to do, so it's a nice touch to have that out of the box.
...
A couple of the special components that Svelte provides are quite handy. `<svelte:head/>` allows you to insert elements into the document head, and things like `<svelte:body/>` give you an easy way to attach event handlers to certain elements outside of your component in a safe way.
